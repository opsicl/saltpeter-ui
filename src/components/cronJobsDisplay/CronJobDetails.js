	  ...
	  <table className="detailsTable">
	    <tbody>
	      ...
	      <tr>
		       ...
		       return <div key={i}> 
				<p className="output" style={{color: "#FF7597u", fontWeight:"bold"}}>{target} :: started at {new Date(this.state.results[target]["starttime"]).toLocaleString()} :: ended at {this.state.results[target]["endtime"] ? new Date(this.state.results[target]["endtime"]).toLocaleString(): "-"} :: ret code {typeof this.state.results[target]["retcode"] === "number" ? this.state.results[target]["retcode"] : "-"} </p> 
